...
class PostProcessor:
    ...
    def __init__(self,file_path = '../GNATS_Server', \
        ...
        if not os.path.exists(self.path_to_csv):
            ...
            if not auto_detect_server_folder_flag:
                print('No GNATS server folder found. Exiting.')
                quit();
            else:
    ...
    '''Change Path if wrong path given for GNATS server'''
        ...
        for fp in self.filepath:
            ...
            for k in range(len(lines)):
                ...
                if 'AC' in lines[k]:            
                    ...
                    tas_knots,heading_deg,fpa_deg,flight_phase'''
                else:
                    pass                      
...
#             axarr[0,0].set_title('time = '+ str(times[0])+' hr' )
    ...
    def plotFinalHistograms(self,vars):
        ...
        for var in vars:
            maxsim = 0.0;
            key = 0;
            for k in list(self.ac_traj_metadata.keys()):
