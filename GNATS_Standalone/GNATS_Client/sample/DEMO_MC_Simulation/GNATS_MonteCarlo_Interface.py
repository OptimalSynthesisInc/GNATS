...
from GNATS_header import GNATS_SIMULATION_STATUS_ENDED, GNATS_SIMULATION_STATUS_PAUSE
import time
import numpy as np
...
class GNATS_MonteCarlo_Interface:
    ...
    def initializeJVM(self, client_dir = './'):
        ...
        self.gnatsClient = GNATSClientFactory.getGNATSClient()
        
        self.equipmentInterface = self.gnatsClient.getEquipmentInterface();


        # Get EnvironmentInterface
        self.environmentInterface = self.gnatsClient.getEnvironmentInterface();
        ...
        self.sim = self.gnatsClient.getSimulationInterface()
                
        self.aircraftInterface = self.equipmentInterface.getAircraftInterface(); 
        
        self.entityInterface = self.gnatsClient.getEntityInterface();
        ...
        self.gnatsClient.login("admin")
        
    
    def shutdownJVM(self):        
        shutdownJVM()
    
    def getGNATSClient(self):
        return self.gnatsClient;
    ...
    def runMCSims(self,args):
        ...
        for var in var_vec:
            ...
            while True:
                ...
                if (server_runtime_sim_status == self.GNATS_SIMULATION_STATUS_ENDED) :
                    break
                else:
                    time.sleep(1)
            #---------PUT THE ABOVE IN EACH PROGRAM------------    
            print("Outputting trajectory data.  Please wait....")
            # The trajectory output file will be saved on GNATS_Server side
#             self.sim.write_trajectories("output_trajectory_" + str(var) + "_" + curr_ac + ".csv")
    ...
    def runMCSimsWithPause(self,args):
        ...
        if var_vec == [[]]:            
            ...
            while True:
                server_runtime_sim_status = self.sim.get_runtime_sim_status()
                if (server_runtime_sim_status == GNATS_SIMULATION_STATUS_PAUSE) :
                    break
                else:
                    time.sleep(1)
