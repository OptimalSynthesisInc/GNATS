...
public class GNATSClient extends BaseClass implements NATSInterface {
	...
	private static final String NATS_CLIENT_VER = "Release 2.0 Linux Distribution";
	private static final String NATS_CLIENT_WIN_VER = "Release 1.0 Windows 10 64bit Distribution";
	...
	private String enV_GNATS_HOME = null;
	...
	private GNATSClient() throws IOException {
		...
		if (mapEnv != null) {
			enV_GNATS_HOME = mapEnv.get("GNATS_HOME");
			
			if (enV_GNATS_HOME != null) {
				enV_GNATS_HOME = enV_GNATS_HOME.trim();
				
				if ("".equals(enV_GNATS_HOME)) {
					enV_GNATS_HOME = null;
				} else if (enV_GNATS_HOME.endsWith("/")) {
					enV_GNATS_HOME = enV_GNATS_HOME.substring(0, enV_GNATS_HOME.length()-1);
				}
			}
		}
    ...
    public void stopStandaloneServer() throws Exception {
    	if (!flag_standalone_mode) {
    		...
    		if (serverProcessId == -1) {
    			...
    			if (Utils.isWindowsPlatform()) {
    				if (enV_GNATS_HOME != null) {
    					Runtime.getRuntime().exec(new String[] {enV_GNATS_HOME + "/utility/stopStandalone.bat", "" + serverProcessId});
    				} else {
    					Runtime.getRuntime().exec(new String[] {"utility/stopStandalone.bat", "" + serverProcessId});
    				}
    			} else {
    				if (enV_GNATS_HOME != null) {
    					Runtime.getRuntime().exec(new String[] {enV_GNATS_HOME + "/utility/stopStandalone.sh", "" + serverProcessId});
    				} else {
