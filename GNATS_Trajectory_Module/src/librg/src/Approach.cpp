...
namespace osi {
map<ProcKey, Approach*> rg_approach;
Approach::Approach() :
		name(""),
		airport(""),
		runway(""),
...
Approach::Approach(const string& name, const string& airport, const string& runway, const map< string, vector<string> >& waypoints,
...
Approach::Approach(const Approach& that) :
	name(that.name),
...
Approach& Approach::operator=(const Approach& that) {
	if(this == &that) return *this;
	this->name = that.name;
	this->airport = that.airport;
	this->runway = that.runway;
...
void Approach::getEntryPoints(vector<string>* const entryNames) {
	if(!connectivity) {
		buildConnectivity();
	}
...
void Approach::destroyConnectivity() {
	if(connectivity) {
...
void Approach::buildConnectivity() {
	destroyConnectivity();
