...
// global sids map declared extern in Sid.h
map<ProcKey, Sid*> rg_sids;
...
Sid::Sid(const string& name, const string& airport, const map< string, vector<string> >& waypoints,
...
Sid& Sid::operator=(const Sid& that) {
...
void Sid::getExitPoints(vector<string>* const exitNames) {
	...
	for(size_t i=0; i<connectivitySize; ++i) {
		...
		if(rowsum == 0) {
			string endname = connectivityNames.at(i);
			exitNames->push_back(endname);
		}
	}
}

void Sid::destroyConnectivity() {
	if(connectivity) {
