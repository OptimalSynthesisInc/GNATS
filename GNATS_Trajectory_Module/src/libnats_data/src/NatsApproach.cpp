...
NatsApproach::NatsApproach(const NatsApproach& that) :
  id(that.id),
  name(that.name),
...
bool NatsApproach::operator<(const NatsApproach& that) const {
...
size_t NatsApproach::size() const {
	return sizeof(NatsApproach) + waypoints.size()*sizeof(string) +
