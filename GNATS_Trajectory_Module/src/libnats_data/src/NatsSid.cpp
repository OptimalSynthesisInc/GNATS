...
NatsSid::NatsSid(const NatsSid& that) :
  id(that.id),
  name(that.name),
...
bool NatsSid::operator<(const NatsSid& that) const {
...
size_t NatsSid::size() const {
	return sizeof(NatsSid) + waypoints.size()*sizeof(string) +
