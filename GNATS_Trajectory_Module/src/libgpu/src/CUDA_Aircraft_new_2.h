...
#ifndef CUDA_AIRCRAFT_NEW_2_H_
#define CUDA_AIRCRAFT_NEW_2_H_
...
//	vector<real_t>           course_rad_taxi;
	...
	__host__ __device__ CUDA_Aircraft_new_2();
//	__host__ CUDA_Aircraft_new_2(const CUDA_Aircraft& that);
	__host__ __device__ ~CUDA_Aircraft_new_2();

	__host__ __device__ float getValue(int idx);
	__host__ __device__ void setValue(int idx, float value);
